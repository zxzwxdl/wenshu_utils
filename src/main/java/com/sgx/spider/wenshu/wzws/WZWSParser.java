...
public class WZWSParser {
    ...
    private final static Pattern jsPattern = Pattern.compile("<script type=\"text/javascript\">(?<js>.+)</script>", Pattern.DOTALL);
    private final static String domain = "http://wenshu.court.gov.cn";

    public static String parse(String text) throws Exception {
        String js = matchJavascript(text);
        ...
        redirectUrl = domain + path;
        return redirectUrl;
    }

    private static String matchJavascript(String text) throws Exception {
        Matcher m = jsPattern.matcher(text);
        if (m.find()) {
            return m.group("js");
        } else {
            throw new Exception("没有可以匹配的script标签，请检查text");
        }
    }
}
